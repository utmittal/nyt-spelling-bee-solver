...
type RecursiveStringDict = dict[str: RecursiveStringDict | None]


def _preprocess_get_prefix_graph_inception(word_list: list[str], curr_dict: RecursiveStringDict):
    """
    Converts the list of words into a directional graph where each node is a string prefix and each path represents a
    letter. I.e. Node 'ro' will have a path 'b' to 'rob' and a path 't' to 'rot' etc. This is the same as the graph
    generated by _preprocess_get_prefix_graph() but we use a different data structure to represent the graph.

    The graph is represented by a nested dictionary where the key is a letter and the corresponding value is either
    ...
    for word in word_list:
        ...
        if first_char in curr_dict:
            curr_dict[first_char] = _preprocess_get_prefix_graph_inception([word[1:]], curr_dict[first_char])
        else:
            curr_dict[first_char] = _preprocess_get_prefix_graph_inception([word[1:]], {})

    return curr_dict


def _inception_graph_recursion(prefix, curr_dict, valid_letters):
    valid_words = []
    if "$" in curr_dict:
        valid_words.append(prefix)
    ...
    for c in valid_letters:
        if c in curr_dict:
            valid_words.extend(_inception_graph_recursion(prefix + c, curr_dict[c], valid_letters))

    return valid_words


def get_bee_solutions_nested_graph(center, others, confusing_dict):
    _validate_character_args(center, others)
    ...
    for c in valid_letters:
        if c in confusing_dict:
            valid_bee_words.extend(_inception_graph_recursion(c, confusing_dict[c], valid_letters))
...
print("Nested Graph")
nested_dict = measure_execution_time(_preprocess_get_prefix_graph_inception, words, {})
solution_words = measure_execution_time(get_bee_solutions_nested_graph, today_center, today_others, nested_dict,
                                        iterations=time_iters)
